...
class Charger(Agent):
    ...
    def __init__(self, unique_id, model):
        super().__init__(unique_id, model)
        self.charging = False  # Boolean for whether the agent is charging or not
...
class Roomba(Agent):
    ...
    def move(self):
        ...
        if dirty:
            ...
            # If no dirty cells, choose from unvisited and truly empty cells
            ...
            if cells_to_visit:
                ...
                # If no eligible cells, move randomly to one of the already visited cells in the neighborhood
    ...
    def move_towards_charger(self):
        ...
        if len(path) > 1:
            next_move = path[1]  # The next cell in the path
            self.model.grid.move_agent(self, next_move)
            self.battery -= 1  # Adjust battery for the movement
    ...
    def step(self):
        ...
        if self.battery <= 20:
            ...
            if self.pos == self.posCharging:  # If Roomba reached the charger
                self.charge()
        else:
            self.move()
        ...
        if not dirty:
            self.model.running = False  # Stop the simulation
